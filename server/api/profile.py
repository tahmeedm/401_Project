...
@router.post("/profile/")
def create_profile_endpoint(profile: UserProfile, db: Session = Depends(get_db), user: UserRequest = Depends(get_current_user)):
    try:
        ...
        db_profile = create_profile(db=db, email=user.email, profile=new_profile)
...
@router.get("/profile/")
def get_profile_endpoint(db: Session = Depends(get_db), user: UserRequest = Depends(get_current_user)):
    email = user.email
