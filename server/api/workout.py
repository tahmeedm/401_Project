...
from crud import get_profile, get_current_user, create_workout_plan, update_workout_plan, get_workout_plan
...
def create_workout(workout: WorkoutPlanModel, db: Session = Depends(get_db), user: UserRequest = Depends(get_current_user)):
    ...
    if check_workout_plan:
        update_workout_plan(db, user_email, workout_plan.preferences.dict(), generated_workout)
        return {"message": "Workout plan updated successfully", "workout_plan": workout_plan}
    
    db_workout_plans = create_workout_plan(db, user_email, workout_plan.preferences.dict(), generated_workout)

    
    return {"message": "Workout plan created successfully", "workout_plan": db_workout_plans}
# Get Workout Plan
@router.get("/workout-plan/")
def get_workout(db: Session = Depends(get_db), user: UserRequest = Depends(get_current_user)):
    user_email = user.email
...
def update_workout(db: Session = Depends(get_db), user: UserRequest = Depends(get_current_user)):
